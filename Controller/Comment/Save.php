...
namespace Acme\FahimNews\Controller\Comment;

use Acme\FahimNews\Controller\Comment;
...
/**
 * Class Save
 * @package Acme\FahimNews\Controller\Comment
 */
    ...
    public function execute()
    {
        ...
        if ($data && $data['comment']) {
            ...
            $autoConfirm = $this->_dataHelper
                ->getStoreConfig('acme_fahimnews/comments/autoapprove');
            $loginApprove = $this->_dataHelper
                ->getStoreConfig('acme_fahimnews/comments/loginapprove');
            $sendEmail = $this->_dataHelper
                ->getStoreConfig('acme_fahimnews/comments/emailoncomment');
            $adminEmail = $this->_dataHelper
                ->getStoreConfig('acme_fahimnews/comments/admin_email');
            $useCaptcha = $this->_dataHelper
                ->getStoreConfig('acme_fahimnews/comments/use_captcha');
            $googleSecretKey = $this->_dataHelper
                ->getStoreConfig('acme_fahimnews/comments/google_secret_key');
            $captchaFlag = false;
            if ($useCaptcha == 1) {
                $captchaFlag = true;
                if (isset($data['g-recaptcha-response'])) {
                    ...
                    $gParams = "secret=" . $googleSecretKey .
                        "&response=" . $response .
                        "&remoteip=" . $remoteIp . "";
                    $googleResponse = $this->_fileSystem->fileGetContents($gUrl . $gParams);
                ...
                if ($sendEmail == 1) {
                    ...
                    $transport = $this->_transportBuilder
                        ->setTemplateIdentifier(
                            'acme_fahimnews_comments_email_template'
                        )
                        ->setTemplateOptions(
                            [
