...
namespace Acme\FahimNews\Helper\Index;
...
/**
 * Class View
 * @package Acme\FahimNews\Helper\Index
 */
class View extends AbstractHelper
{
    ...
    public function initProductLayout(ResultPage $resultPage, $layoutId)
    {
        $postListLayout = $this->_dataHelper
            ->getStoreConfig('acme_fahimnews/general/' . $layoutId);
    ...
    public function prepareAndRenderPost(ResultPage $resultPage, $postId, $pageNo)
    {
        ...
        $resultPage->addHandle('fahimnews_post_view');
        ...
        $typeOfComment = $this->_dataHelper
            ->getStoreConfig('acme_fahimnews/comments/type_of_comment');
        if ($typeOfComment) {
            // echo $currentPage;exit;
            $listBlock = $resultPage->getLayout()->getBlock('fahimnews.comment');
            // $listBlock->setCurrentPage($currentPage);
        }
        return $this;
    }
